...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void checkNicknameAvailable() {
    ...
    bool isAvailable = isValidNickname(
        enteredNickname); // 사용자가 입력한 닉네임을 심심이와 비교

    setState(() {
      // 닉네임 사용 가능 -> 사용 가능 메시지 출력 -> 중복 확인 버튼 비활성화 -> 비밀번호 텍스트 필드 입력 가능
      if (isAvailable) { // 닉네임 텍스트 필드 입력시
  ...
  void updatePasswordState() {
    setState(() {
      if (isNicknameButtonClickable &&
          nicknameAvailableText == "사용 가능한 닉네임이에요.") { // 중복확인 버튼 비활성화
        isNicknameButtonClickable = false;
                          ...
                          child: TextField(
                            ...
                            onChanged: (text) {
                              if (text.length < minPasswordCheckLength) {
                                ...
                                print(
                                    '최대 $maxPasswordCheckLength자만 입력할 수 있습니다.');
                              }
                            },
